...
static void do_search(
    ...
    /* Start a timer thread that sets ctx.stop mid-search when movetime elapses.
     * eval_ctx checks ctx.stop at every node, so the search stops within one
     * node evaluation rather than waiting for the current depth to complete. */
    std::atomic<bool> timer_cancel{false};
    std::thread timer_thread;
    if(movetime_ms > 0){
        timer_thread = std::thread([&ctx, &timer_cancel, movetime_ms, &search_start](){
            auto deadline = search_start + std::chrono::milliseconds(movetime_ms);
            while(!timer_cancel.load(std::memory_order_relaxed)){
                auto now = std::chrono::high_resolution_clock::now();
                if(now >= deadline){
                    if(!timer_cancel.load(std::memory_order_relaxed)){
                        ctx.stop = true;
                    }
                    return;
                }
                ...
                std::this_thread::sleep_for(
                    std::chrono::milliseconds(std::min<int64_t>(10, remaining)));
            }
        });
    }
    ...
    for(int depth = 1; depth <= depth_limit; depth++){
        ...
        if(movetime_ms > 0 && total_ms >= movetime_ms){
            break;
        }
